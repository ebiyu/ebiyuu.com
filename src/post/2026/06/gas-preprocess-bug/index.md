...
Google Apps Script のウェブアプリ（`createHtmlOutputFromFile`）のインライン `<script>` のパースにバグを見つけたのでメモ。
...
次の3つの要素が揃うと、正常に動作しないJavaScriptコードが配信される。

1. オブジェクトリテラル `{ ... }` の中であること
2. その値に、`/` を含むテンプレートリテラルがあること（例：`` `${a}/${b}` ``）
3. その**後方**に、別の `/` を含む文字列の値があること（例：`"c/d"`）

テンプレートリテラル内の `/` のパースにバグがあることで、後ろにある別の文字列（`"https://..."` など）の `//` を行コメントと誤認して行末ごと削除してしまう。
...
ウェブアプリをデプロイして開くと、本来は `https://example.com/a/bc/d` の表示が期待されるところ、画面が出ず、devtoolを見ると `SyntaxError` が発生している。
...
`return "https:` しか見えないことから、 `//example.com/...` 以降が消えていると推測される。


### 直し方

テンプレートリテラル内に `/` が入らないようにすればよい。
